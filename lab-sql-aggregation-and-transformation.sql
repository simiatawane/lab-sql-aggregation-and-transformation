...
SELECT CONCAT( first_name, ' ' , last_name, ' @ ' , LEFT( email , 3 ) ) AS address FROM sakila.customer;
...
SELECT rating, round(AVG(length),2) AS mean_film_duration FROM sakila.film GROUP BY rating ORDER BY mean_film_duration DESC;
...
SELECT rating, round(AVG(length),2) AS mean_film_duration FROM sakila.film GROUP BY rating HAVING mean_film_duration > 120 ORDER BY mean_film_duration DESC;
